...
def corp_market(cls):
    return {"Y": "KOSPI", "K": "KOSDAQ", "N": "KONEX"}.get(cls, "")
...
def fetch_dart_market(api_key, corp_cls, start_yyyymmdd, end_yyyymmdd):
    ...
    while page_no <= max_pages:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "StockNewsRadar/1.1"})
        ...
        if status == "013":
            break
        if status != "000":
            raise RuntimeError(f"OpenDART 오류 {status}: {payload.get('message', '알 수 없는 오류')}")
...
def build_live_candidates(raw_rows):
    ...
    for row in raw_rows:
        ...
        candidates.append({
            ...
            "summary": report_nm,
            "reason": info["reason"],
            "source": f"OpenDART · {corp_market(row.get('corp_cls'))}",
            "published_at": row.get("rcept_dt") or "",
            "url": f"https://dart.fss.or.kr/dsaf001/main.do?rcpNo={rcept_no}",
        })
    candidates.sort(key=lambda x: (x["material_score"], x["published_at"]), reverse=True)
    return candidates
...
def render_card(x):
    ...
    link_html = f'<a class="source-link" href="{safe_url}" target="_blank">DART 원문 보기 ↗</a>' if safe_url else ""
...
st.markdown(
    f"""
    <div class="hero">
      <div class="hero-title">StockNewsRadar <span class="live-badge">LIVE V1.1</span></div>
...
market_label = st.segmented_control("시장", options=["한국", "미국"], default="한국", label_visibility="collapsed")
...
if market_label == "한국":
    ...
    try:
        with st.spinner("OpenDART에서 KOSPI · KOSDAQ · KONEX 공시를 확인하고 있습니다..."):
            raw = []
            for cls in ("Y", "K", "N"):
                raw.extend(fetch_dart_market(api_key, cls, start_date, end_date))
        candidates = build_live_candidates(raw)
    except Exception as exc:
        st.error(f"OpenDART 연결 오류: {exc}")
        st.info("인증키 또는 OpenDART 서비스 상태를 확인하세요.")
        st.stop()
    ...
    st.markdown(
        ...
        <div class="market-strip">
          ...
          <div class="market-chip"><div class="k">데이터</div><div class="v pos">OpenDART LIVE</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
- **잠정 실적·합병·분할 등**은 제목만으로 호재/악재를 단정하지 않습니다.
- **실제 매수 추천이 아닙니다.** 다음 거래일에 먼저 조사할 종목을 좁히는 단계입니다.
        """)
else:
